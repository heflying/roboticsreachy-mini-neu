...
In headless mode, there is no Gradio UI. If the selected backend API key is not
...
class LocalStream:
    ...
    def _active_backend(self) -> str:
        """Return the configured realtime backend."""
        return get_backend_choice()

    def _required_api_key_name(self) -> str:
        """Return the env var name needed for the selected backend."""
        return "DASHSCOPE_API_KEY" if self._active_backend() == QWEN_OMNI_BACKEND else "OPENAI_API_KEY"

    def _has_required_api_key(self) -> bool:
        """Return True when the selected backend has the key it needs."""
        if self._active_backend() == QWEN_OMNI_BACKEND:
            return bool(config.DASHSCOPE_API_KEY and str(config.DASHSCOPE_API_KEY).strip())
            ...
            return JSONResponse(
                {
                    ...
                    "has_key": self._has_required_api_key(),
                    "required_key": self._required_api_key_name(),
                }
            )
    ...
    def launch(self) -> None:
        ...
        # If the OpenAI key is still missing, try to download one from HuggingFace.
        # Qwen/DashScope keys must be provided explicitly by env or instance .env.
        if self._active_backend() == OPENAI_BACKEND and not self._has_required_api_key():
        ...
        if not self._has_required_api_key():
            key_name = self._required_api_key_name()
            if self._active_backend() == QWEN_OMNI_BACKEND:
                logger.warning("%s not found. Configure it in the instance .env.", key_name)
            else:
            ...
            try:
                while not self._has_required_api_key():
                    refresh_runtime_config_from_env()
                    time.sleep(0.2)
