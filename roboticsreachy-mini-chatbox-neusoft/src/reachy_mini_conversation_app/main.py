...
def run(
    ...
    # Default: cascade mode. Use --realtime to switch to realtime audio-to-audio backends.
    ...
    if is_qwen_omni_model():
        from reachy_mini_conversation_app.qwen_omni_realtime import QwenOmniRealtimeHandler

        logger.info("Using Qwen Omni Realtime handler for model: %s", config.MODEL_NAME)
        handler = QwenOmniRealtimeHandler(deps, gradio_mode=args.gradio, instance_path=instance_path)
    else:
    ...
    if args.gradio:
        uses_qwen_backend = is_qwen_omni_model()
        key_label = "DASHSCOPE_API_KEY" if uses_qwen_backend else "OPENAI API Key"
        key_value = (
        ...
        api_key_textbox = gr.Textbox(
            label=key_label,
            type="password",
            value=key_value if not get_space() else "",
        )
