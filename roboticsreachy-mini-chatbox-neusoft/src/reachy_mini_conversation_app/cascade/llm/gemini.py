...
class GeminiLLM(LLMProvider):
    ...
    async def warmup(
        ...
        This sends a complete request to trigger prompt processing.
        ...
        Note:
            The warmup uses max_output_tokens=1 to minimize cost while still
            triggering the prompt processing. The generated token is discarded.
        """
        try:
            if messages is None:
                messages = []

            logger.info(f"Warming up Gemini with {len(messages)} messages, {len(tools) if tools else 0} tools")
            ...
            # Build config with max_output_tokens=1
    ...
    async def generate(
        ...
        try:
            ...
            if token and token.cancelled:
                logger.info("Gemini generation skipped for cancelled turn %s", token.turn_id)
                return
            ...
            try:
                async for chunk in stream:
                    if token and token.cancelled:
                        logger.info("Gemini stream cancelled for turn %s", token.turn_id)
                        return
            ...
            if token and token.cancelled:
                logger.info("Gemini generation aborted before completion for turn %s", token.turn_id)
                return
