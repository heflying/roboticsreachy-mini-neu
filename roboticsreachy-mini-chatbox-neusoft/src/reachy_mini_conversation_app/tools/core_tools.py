...
def _load_profile_tools() -> None:
    ...
    tool_names.extend(tool_name for tool_name in CORE_MEMORY_TOOL_NAMES if tool_name not in tool_names)
...
def get_tool_specs(exclusion_list: list[str] = []) -> list[Dict[str, Any]]:
    ...
    excluded = set(exclusion_list)
    if not memory_command_writes_enabled():
        excluded.update(CORE_MEMORY_TOOL_NAMES)
